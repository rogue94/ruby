class Personne
  ...
  def info
    ...
    if @en_vie
	return "#{@nom} [PV: #{@points_de_vie}/100]" 
  ...
  def attaque(personne)
    ...
    puts "\n#{personne.nom} se fait attaquer par #{nom}"
  ...
  def subit_attaque(degats_recus)
    ...
    # - Affiche ce qu'il s'est passé
    puts "#{nom} a reçu #{degats_recus} PV de degats"
...
class Joueur < Personne
  ...
  def degats
    ...
    degats += degats_bonus
    # - Affiche ce qu'il s'est passé
    puts "#{nom} a provoqué #{degats} points de dégats à son adversaire"
    return degats
  end

  def soin
    # A faire:
    # - Gagner de la vie (amélioration = 10 // PV ne peuvent pas être > 100 PV)
    soin = 10
    if @points_de_vie < 90
       @points_de_vie += 10
       puts "#{nom} prend une potion et augmente ses PV à #{@points_de_vie} PV (Soin: #{soin} PV)"
    else
       @points_de_vie = 100
       puts "#{nom} a augmenté ses PV au maximum (100 PV) - pas besoin de toute la potion !"
    end
    # - Affiche ce qu'il s'est passé
    #puts "#{nom} prend une potion et augmente ses PV (de #{soin} à #{@points_de_vie} PV)"
  end
  ...
  def ameliorer_degats
    ...
    # - Affiche ce qu'il s'est passé
    puts "#{nom} fait une attaque exceptionelle et augmente les dégats de #{degats_bonus}"
    return degats_bonus
  end
end
...
class Monde
  ...
  def ennemis_en_vie
    ...
    @ennemis.each do |ennemi|
      arr_ennemis_en_vie << ennemi if ennemi.en_vie
    end
    arr_ennemis_en_vie
  end
end
...
100.times do |tour|
  ...
  elsif choix == 99 || (choix > 1+monde.ennemis.size) 
    # Si on a choisi une option non présent 
    # renvoi vers le prochain tour
    if choix < 99
      puts "Le choix #{choix} n'existe pas !!! Rendez-vous au prochain tour!"
      next
    end
